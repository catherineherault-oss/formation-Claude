...
- échelle logarithmique (correcte pour un OR) ;
- regroupement par bloc de variables (sexe / âge / CSP / diplôme / continues) ;
- valeurs OR [IC 95 %] annotées à droite ; points pleins = significatifs à 5 %.
N&B pour Décisions Marketing. Sortie : figures/png/fig4_regression_or_log_nb.png (+ svg).
"""
...
# ---- Construction des lignes (haut vers bas) -------------------------------
rows=[]   # (kind, label, or, lo, hi, p)
...
n=len(rows); ypos=list(range(n))[::-1]   # ligne 0 en haut

fig,ax=plt.subplots(figsize=(8.8,0.42*n+1.2))
for (kind,label,o,lo,hi,p),y in zip(rows,ypos):
    if kind!="data": continue
    sig=p<0.05
    ax.plot([lo,hi],[y,y],"-",color="black",lw=1.1,zorder=2)
    for xb in (lo,hi): ax.plot([xb,xb],[y-0.12,y+0.12],"-",color="black",lw=1.1)
    ax.plot(o,y,"o",ms=6,mfc=("black" if sig else "white"),mec="black",mew=1.2,zorder=3)

ax.axvline(1,color="black",lw=0.9,ls="--",zorder=1)
ax.set_xscale("log")
ax.set_xlim(0.4,3.6)
...
ax.set_xlabel("Odds-ratio (échelle logarithmique) — point plein : significatif à 5 %")

ax.set_yticks(ypos)
ylabels=[("" if k=="header" else "   ")+lab for (k,lab,*_ ) in rows]
...
ax.text(1.02,n-0.2 if False else ypos[0]+0.0,"OR [IC 95 %]",transform=trans,
        fontsize=8.5,fontweight="bold",va="center",ha="left")
for (kind,label,o,lo,hi,p),y in zip(rows,ypos):
    if kind!="data": continue
    star=" *" if p<0.05 else ""
    ax.text(1.02,y,f"{o:.2f} [{lo:.2f}–{hi:.2f}]{star}",transform=trans,
            fontsize=8.5,va="center",ha="left")

ax.set_title("Déterminants de l'usage de la vente directe (odds-ratios)",fontsize=11)
fig.text(0.01,0.005,f"Régression logistique · N = {int(m.nobs)} · pseudo-R² (McFadden) = {m.prsquared:.3f}",
...
print("Forest plot amélioré généré : figures/png/fig4_regression_or_log_nb.png")
